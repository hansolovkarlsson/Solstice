        ...
        case NODE_BUILTIN_CALL:
            if (node->op == TOKEN_EOF_FN || node->op == TOKEN_EOLN) {
                ...
                if (node->op == TOKEN_EOF_FN) {
                    // A typed (or untyped) file needs a genuinely
                    // separate opcode - text mode's OP_EOF_FILE is a
                    // byte-level fgetc+ungetc peek, not meaningful for
                    // binary records (parse time already rejects 'eoln'
                    // on a typed/untyped file entirely, so no
                    // OP_TYPED_FILE_EOLN exists). OP_TYPED_FILE_EOF
                    // itself is a pure byte-position comparison (ftell/
                    // fseek, no record_byte_size involved at all - see
                    // vm.c), so it's already exactly as meaningful for
                    // an untyped file as a typed one.
                    if (file_idx != -1 && (sym_table[file_idx].type == TYPE_TYPED_FILE
                                            || sym_table[file_idx].type == TYPE_UNTYPED_FILE)) {
        ...
        case NODE_FILE_OP:
            if (node->op == TOKEN_FILE_ASSIGN) {
                ...
                if (sym_table[node->data.var_idx].type == TYPE_TYPED_FILE
                    || sym_table[node->data.var_idx].type == TYPE_UNTYPED_FILE) {
                    ...
                    // here. An UNTYPED file packs 0 (see parser.c's own
                    // comment on this) - reused purely for the binary-
                    // mode "rb"/"wb" fopen behavior this opcode already
                    // has.
