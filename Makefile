...
.PHONY: all clean pascalc solvm solas desole test_recovery

all: $(BINDIR)/pascalc $(BINDIR)/solvm $(BINDIR)/solas $(BINDIR)/desole $(BINDIR)/test_recovery
...
HEADERS := $(wildcard src/common/*.h src/pascalc/*.h src/solvm/*.h src/solas/*.h src/desole/*.h)
$(PASCALC_OBJS) $(SOLVM_OBJS) $(SOLAS_OBJS) $(DESOLE_OBJS) $(TEST_RECOVERY_OBJS): $(HEADERS)

clean:
	rm -f $(BINDIR)/pascalc $(BINDIR)/solvm $(BINDIR)/solas $(BINDIR)/desole $(BINDIR)/test_recovery
	rm -f $(COMMON_OBJS) $(FRONTEND_OBJS) $(VM_OBJS) \
	      src/pascalc/pascalc.o src/solvm/solvm.o src/solas/solas.o src/desole/desole.o \
	      src/pascalc/test_recovery.o
